...
const getListMusic = asyncHandler(async (req, res) => {
    try {
        // Lấy danh sách bài hát và sắp xếp theo thời gian tạo (mới nhất trước)
        const listMusic = await MusicModel.find().sort({ createdAt: -1 });
        ...
        res.status(500).json({
            success: false,
            message: "Error fetching music list",
            error: error.message,
        });
    }
});
